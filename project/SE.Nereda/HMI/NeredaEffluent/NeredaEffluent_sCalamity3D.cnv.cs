/*
 * Created by EcoStruxure Automation Expert.
 * User:  
 * Date: 7/11/2026
 * Time: 9:50 PM
 * 
 */

using System;
using NxtControl.GuiFramework;
using System.ComponentModel;

namespace SE.Nereda.Symbols.NeredaEffluent
{
	/// <summary>
	/// Description of sCalamity3D.
	/// </summary>
	public partial class sCalamity3D : NxtControl.GuiFramework.HMISymbol
	{
		private string _CALAMITY = "Name";
		float Percent;
		public sCalamity3D()
		{
			//
			// The InitializeComponent() call is required for Windows Forms designer support.
			//
			InitializeComponent();
			WaterLevel.FillPercent = 0;
			WaterBase.Visible = false;
			WaterTop.Visible = false;
		}
		
		[RefreshProperties(RefreshProperties.Repaint)]
		[Category("Tagnames")]
		[DisplayName("Name Code")]
		
		public string _iCALAMITY
		{
			get{
				return _CALAMITY;
			}
			set{
				_CALAMITY = value;
				CALAMITY.Text = value.ToString();
			}
		}

		void SPvBarVer1ShareLevelPv(float ShPvMax, float ShPvMin, float ShPv)
		{
			// TODO: Implement SPvBarVer1ShareLevelPv
			Percent = (float) (ShPv / ShPvMax) *100;
			if(Percent > 0){
				WaterBase.Visible = WaterTop.Visible = true;
			}
			else if(Percent == 0){
				WaterBase.Visible = false;
				WaterTop.Visible = false;
			}
			
			WaterLevel.FillPercent = Percent;
			WaterTop.Location = new NxtControl.Drawing.PointF(32, 232 + (Percent * (96 - 232) / 100));
		}
	}
}
